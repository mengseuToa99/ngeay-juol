...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ __('ngeay juol') }} - {{ __('Reset Password') }}</title>
        <link class="icon" href="{{ asset('favicon.ico') }}" sizes="any">
        <link class="icon" href="{{ asset('Khmer%20House%20Key.svg') }}" type="image/svg+xml">
        ...
        <div class="w-full max-w-5xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200/50 dark:border-slate-800/50 overflow-hidden grid grid-cols-1 lg:grid-cols-12 relative z-10 min-h-[600px]">
            ...
            <div class="lg:col-span-5 bg-gradient-to-tr from-slate-900 via-slate-850 to-slate-900 p-8 lg:p-12 text-white flex flex-col justify-between relative overflow-hidden">
                ...
                <div class="my-auto py-8 relative z-10">
                    ...
                    <p class="text-slate-400 text-sm leading-relaxed mb-8">
                        {{ __('Ensure your new password contains a mix of characters to keep your account secure. Once updated, you will be redirected to the sign in page.') }}
                    </p>
                </div>
            ...
            <div class="lg:col-span-7 p-8 lg:p-16 flex flex-col justify-center relative">
                ...
                <div class="max-w-md w-full mx-auto">
                    <!-- Heading -->
                    <h1 class="font-display font-bold text-3xl mb-2">{{ __('Reset Password') }}</h1>
                    <p class="text-slate-500 dark:text-slate-400 text-sm mb-8">{{ __('Create a new secure password for your ngeay juol account.') }}</p>
                    ...
                    @if ($errors->any())
                        <div class="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-sm flex items-start gap-2.5">
                            ...
                            <div>
                                <span class="font-semibold">{{ __('Reset failed') }}</span>
                    ...
                    <form method="POST" action="{{ route('password.update') }}" class="space-y-5">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input id="email" name="email" type="email" value="{{ old('email', $request->email) }}" required readonly
                                    class="block w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 text-slate-500 cursor-not-allowed text-sm outline-none">
                            </div>
                        </div>

                        <!-- New Password Field -->
                        <div>
                            <label for="password" class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">{{ __('New Password') }}</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                        ...
                        <div>
                            <label for="password_confirmation" class="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">{{ __('Confirm New Password') }}</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                                </div>
